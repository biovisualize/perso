...
'-----------------------------for Power-Basic----------------''$INCLUDE "plibxy-p.inc"
programname$ = "XY"
basfilename$ = "DXY.BAS"
ptype$ = ""
dirtemp$ = "..\tmp\"
...
CALL clsscr(ipcs, ipcx, ibackcol)
 END IF
IF ipcontrol = 0 THEN
              'CALL showdisplay(kd, displaywastmp$)
...
CASE 7: diff = DG
END SELECT
sum1 = 0: sum2 = 0   'tip at js - averaging for head in hydra
	FOR iy = 1 TO jy
...
IF K3 = 4 THEN   'tip at kax - averaging for foot in hydra
	sum1 = 0: sum2 = 0
	FOR iy = 1 TO jy
