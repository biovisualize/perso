...
' for storing and retrival of distribution, for removing parts of the field (regeneration )
' for insertion of new cells to simulate growth, modify concentrations.. etc

imxl2 =imxl/2
 mamessage$ = ""
genedw$ = "": ipu = INSTR(displaywas$, "/") 'Not in use here
...
' to be compatible with older Guided Tours
...
CASE "T"
    IF kamem = 0 THEN
      ...
      pcontroloc$ = "S" :  GOTO manipulationstartxt
      END IF
...
CALL cmessage(6, -1, 1, 15, 4, ibackcol, " Sorry, XT-array too large!", "OK")
  ...
  IF genedw$ > "" THEN  'not used in this package
    IF i >= imxl THEN
    CALL cmessage(6, -1, 1, 15, 4, ibackcol, " Sorry, XY-array too large!", "OK")
    END IF
...
CASE ELSE
  ...
  INPUT ; "Type of manipulation"; resp$
...
FOR ix = LX(il) TO kinsert STEP -1          'still the old js is being used!
   ...
   NEXT iy:  NEXT ix
   ' if k1= 12  more cells are inserted on the left side, removed at right
   'number of cells remain consant
...
''shared icol(), iwi(), itypus(), ibm()
''shared ivgashift, msiluet(), msiluetmax, ihp, fdelay, radius, yhzyl, DeltaWi
''shared dxp, dyp, istelz, icc, k1, k2, k3, k4, ihintr
...
IF iy = 1 AND ahidden = 0 THEN '----------- Initialization ---------------------
  ...
  CASE 6,7,8,9,11,12,13, 14'pixelpattern for pixel plots
    ipat(1, 1) = &H4444
    ipat(1, 2) = &H1111
    ipat(1, 3) = &H8888
    ipat(1, 4) = &H2222
...
CASE 1 '3D plot: time = background -> foreground, with hidden lines
  ...
  IF y2 > s2 THEN 'left point is visible
    yb = ya: IF s2 > yb THEN yb = s2
     CALL linep(ihp, x2, yb, x2, y2, 0, iwil, icolor, 1)
  ...
  FOR ix = ja + 1 TO js
	...
	ELSE
     ...
     xend = xanf + xr
     YEND = yanf + yr
     CALL linep(ihp, xanf, yanf, xend, YEND, 0, iwil, icolor, 1)'  line after krot-loop
     xanf = xend
     yanf = YEND
	  xr = 0
	  yr = 0
  ...
  CALL linep(ihp, xend, YEND, xend, ya, 0, iwil, icolor, 1)
...
CASE 2'3D plot: time = background -> forground
  ...
  FOR ix = ja + 1 TO js
	...
	IF y2 > s2 THEN ' point 2 has to be removed too
	  ...
	  yl = y1
	  DO WHILE xl <= x2
	    LINE (xl, yl)-(xl + 1, yma), ibackcol, BF
	    xl = xl + 1
	    yl = yl + ddy
	  LOOP
...
CASE 8, 9, 12, 13, 14  '8 with stable pattern, 10 - 14 for pixel in higher resolution
   FOR ix = ja TO js
    krnd = RND * 3 + 1
      jlocal = axt(il, ix) * ff
      IF jlocal >= 1 THEN
...
CASE 10 'brownish colour
    FOR ix = ja TO js
      jlocal = axt(il, ix) * ff
      IF jlocal > 5 THEN jlocal = 5
      IF jlocal >= 1 THEN LINE (x1, y1)-(x1 + dx, y1-1.5), jlocal, BF
      x1 = x1 + dx
    NEXT ix
  END SELECT
...
CASE 17, 30  'Ring of cells, for organizer
    ibm(1) = icolor
  ...
  stepzahl = 1
	  ...
	  LINE (ixs, iyu)-(ixs+istep, iyo), icow, BF
	  stepzahl = stepzahl + 1
	  NEXT ixs
   END IF
  LINE (ix2, iy2)-(ix2+istelz, iy2h), icow, BF
ELSE 'only line
...
CASE 20
  ...
  ELSE
   IF ya - ym(il) > 0 THEN
   LINE (xo, ym(il) - iwi(il))-(xo + iwi(il), ya + iwi(il)), icolor, BF
   ELSE
   LINE (xo, ym(il) + iwi(il))-(xo + iwi(il), ya - iwi(il)), icolor, BF
  END IF
  END IF
...
CASE 26 'Ring of cells, for organizer
   ...
   wi2 = wi1 + DeltaWi
   icoloc = icolor: wi21 = wi2
   IF ix < js21 AND il = 1 THEN icoloc = 7
   IF ix < js21 AND il = 2 THEN icoloc = 10
   ih = axt(il, ix) * f1
   IF il = 1 THEN
   FOR i = 0 TO ih
   CIRCLE (xo, yhzyl + i), radius, icoloc, wi1, wi2, per
   NEXT
   ELSE
...
CASE 28 'Cylinder for phyllotaxis
   ...
   FOR ix = ja TO js   'drawn  along an elipse if above a threshold
   ixx = ix + ihintr ' ihintr = rotation of the cylinder, in "cells"
   IF ixx > js THEN ixx = ixx - js + ja - 1
   wi2 = wi1 + DeltaWi
'   wiplus = w1 + 1.1*DelataWi Still gaps there
   icoloc = icolor: wi21 = wi2
   IF SIN(wi2) > 0 THEN icoloc = ico2
   IF axt(il, ixx) * ff > 1 THEN
   CIRCLE (xo, yo), radius, icoloc, wi1, wi2, per
   END IF
   wi1 = wi2
   NEXT ix
IF ipcs > -2 THEN
...
CASE 17, 30 'Ring of cells, for organizer
  ...
  stepzahl = 1
	  FOR ixs = ix1 + istep TO ix2 STEP istep
	  iyu = iy1 + dyu * stepzahl: iyo = iy1h + dyo * stepzahl
	  LINE (ixs, iyu)-(ixs+istelz, iyo), icow, BF
	  stepzahl = stepzahl + 1
	  NEXT ixs
   END IF
  LINE (ix2, iy2)-(ix2+istelz, iy2h), icow, BF
ELSE 'only line
    x1 = ix1: y1 = iy1h: x2 = ix2: y2 = iy2h:
    CALL linep(ihp, x1, y1, x2, y2, 0, iwi(il), icow - 20, 1)
END IF
...
IF istelz = 4 THEN  ' add last line of the inhibitor
 ...
 END IF
  ...
  x2 = x1 + pdx * (js - ja)
  y2 = y1 - pdyx * (js - ja)
  CALL linep(ihp, x1, y1, x2, y2, 0, iwil, icolor, 1)
  x3 = x2 + (iy - 1) * px
  y3 = y2 + pyy * (iy - 1)
  CALL linep(ihp, x2, y2, x3, y3, 0, iwil, icolor, 1)
  x4 = x3 - pdx * (js - ja)
  y4 = y3 + pdyx * (js - ja)
...
''shared itot&, label$(), title$, displaywas$, icheckerror, lpt$
...
DIM am(10)
ipattern = 0
IF icc = 1 THEN   '  If igraph 5-7 windows = screen resolution as VGA resultion
                  ' for KD 6-14 always pixel resultion required, e.g. also in screen 3
      SELECT CASE igraph'  units=screen-pixel
      case 1 to 4, 9,12: xmax = 648: ymax = 480: WINDOW (0, 0)-(xmax, ymax)
      CASE 5: xmax = 800: ymax = 600: WINDOW (0, 0)-(xmax, ymax)
      CASE 6: xmax = 1024: ymax = 786: WINDOW (0, 0)-(xmax, ymax)
      CASE 7: xmax = 1280: ymax = 1024: WINDOW (0, 0)-(xmax, ymax)
    END SELECT
  end if
SELECT CASE inz
CASE 0
...
CASE 4, 5'----- schematic plot with letters --------
 ...
 color 1,15
  ...
  LOCATE 1, 1
IF inz = 555 THEN
    CALL zeingabe(igt, 0, 3, i, d, resp$, "filename for saving distributions (use [drive] to change drive) " + dirtemp$)
	  IF resp$ = "" OR resp$ = CHR$(27) THEN EXIT SUB
	  icheckerror = 0
 ...
 LOCATE 1, 1
  ...
  FOR ilv =  1 to LEN("S"+dw$+"h")
    ...
    IF il = 19 THEN il = 0
   color ico(il) MOD 20,15
    if il = 0 or il = 8 then color 1,15
...
CASE 6 TO 14'----- pixel-plot, 22 bottom up for phyllotaxis
  IF icc = 1 THEN
    x1 = 1 + dx * ja + gxa(1)
    x2 = 1 + dx * js + gxa(1)
    y1 = ymax - 4: IF inz = 14 THEN y1 = 24
    ...
    IF inz = 8 OR inz = 12 THEN
      ...
      xm = xz: ym = yz
      NEXT i
      y1 = y1 - 10 ' new sarting point of the plots
   END IF
   END IF ' setting initials
   ydiff = 1: IF inz = 14 THEN ydiff = -1
	...
	CALL linep(ihp, x1 - iwi(1), y1 - ydiff, x2 + iwi(1), y1 - iwi(1) * ydiff, 2, 1, ivorcol, 1)
        iendm = 999
   ELSE
    IF y1 < 23 THEN EXIT SUB 'reaching the lower end of the screen
...
il = 1
    CALL prof(axt(), il, dw$, 0, inz, iendm, dx, gfa(il), ja, js, icc, gxa(il), gya(il))
...
IF inz = 16 OR icc = 1 THEN CALL clearscreen
  ...
  radius = dx/4 * (js - ja)
 axt(0, js + 1) = axt(0, ja)' otherwise source would be undefined
...
'18 below each other, cls only at icc = 1
  IF iendm = 99 THEN EXIT SUB
  IF icc = 1 OR KG > 0 THEN
  jam = ja
  IF gxa(1) = 0 THEN
...
CASE 20 'for display of oscillation within a single cell
  IF icc = 1 THEN
    ...
    LINE (x1 - 2, y2)-(x1+1, y1 - 4), ivorcol, BF 'first line perpendicular
  ELSE
    x1 = x1 + 1: LINE (x1, y1 - 2)-(x1+3, y1 - 4), ivorcol, BF  ' elongation of the base line
     END IF
  FOR ilv = LEN(dw$) TO 1 STEP -1
    il = ASC(MID$(dw$, ilv, 1)) - 96'a=asc(97)
    IF il = 19 THEN il = 0
...
from19to25:
  ...
  FOR ix = ja TO js
    ...
    NEXT ilv
      ...
      anc = 0
  IF su > .01 THEN
      FOR ibc = 1 TO ill'normalize
      	IF su < 1! THEN
	  anc = anc + am(ibc)
	ELSE
	  anc = anc + am(ibc) / su
	END IF
   agfa(ibc) = anc'afraction of the sum
      NEXT ibc
...
CASE 28'plot as cylinder, for phyllotaxis
  ...
  ikringel = istelz: IF ikringel < 8 THEN ikringel = 8
  y1 = gya(1)
  x1 = gxa(1): icountl = 0
  radius = dx * (js - ja)
  IF gxa(1) = 0 THEN x1 = 319: y1 = 40
  END IF
  icountl = icountl + 1
  IF icountl > ikringel THEN 'yellow circle to illustrate the cylinder
   CIRCLE (x1, y1), radius, 7, , , per
   icountl = 0
  END IF
   y1 = y1 + 1
      FOR ilv = LEN(displaywas$) TO 1 STEP -1
...
SELECT CASE inz
CASE 191  ' shift dowwards
   yo = gya(1) - istelz - 20
   GOTO from19to25
CASE 27: radius = .995 * radius 'Next plot a bit smaller with kd=27
END SELECT
