...
''SHARED ihp, dx, dy, dz, per, LABEL$()
''SHARED istelz, iletztk, ihintr, krot
...
CASE 11, 17' colored random dots, density proportional to level
  IF xo = 0 THEN
  IF icc = 1 OR kg > 0 THEN
  xa = 320 - ((js - ja + 1) / 2) * dx
  jam = ja
...
frame:
 ...
 yos = yo: IF kdisplay = 5 OR kdisplay = 6 THEN yos = yo - (pyy + pdyx) / 2
  ...
  END SELECT
  IF istelz > 0 AND iletztk > 10 THEN    'vertical dotted lines between distributions
    stelzm = iletztk - y1
...
IF js < imx - 1 THEN
FOR kil = 1 TO kn
FOR ix = lx(kil) TO iinsert STEP -1          'still the old js is being used
     FOR iy = 1 TO ly(kil)
      axy(kil, ix + 1, iy) = axy(kil, ix, iy)
   NEXT iy:  NEXT ix
   lx(kil) = lx(kil) + 1
   NEXT kil
js = js + 1
...
CASE "S"        'to store present state of cells in memory arrays
 IF 2 * jy > imy THEN
  ...
  CALL cmessage(4, -1, 1, 15, 12, ibackcol, mess$, "OK")
  EXIT SUB
 END IF
...
CALL zeingabe(igt, 1, 1, kanew, dummy, dummy$, "x-size is "+ STR$(js)+"; X-left-most to be deleted")
CALL zeingabe(igt, 1, 1, ksnew, dummy, dummy$, "X-right-most cell to be deleted")
...
CALL zeingabe(igt, 1, 1, kaynew, dummy, dummy$, "y-size is "+ STR$(jy)+"; y-left-most to be deleted")
CALL zeingabe(igt, 1, 1, kynew, dummy, dummy$, "Y-right-most cell to be deleted")
...
CASE "GR", "T"'graft pieces together
	 ...
	 mempos(ix) = 0 'first mop up anything left from previous manipulations
      ...
      PRINT " You can combine cells from the stored state to create a new field. The fragments are part"
      PRINT " of the X-extension and full Y extension. Each cell can be used in the new field several"
      PRINT "  times. The stored field has"; kxmem; "cells."
      PRINT " If you want to take just one row of cells return two identical positions. ";
      END IF
graftarray:
  mess$ = " starting x-row (RETURN = end of grafting process, ESC=nothing)"
  ifrom = -1
  CALL zeingabe(igt, 0, 1, ifrom, dummy, dummy$, mess$)
  IF ifrom = -1 THEN exit sub
  IF ifrom = 0 THEN GOTO graftassembly
  mess$ = "  end cell of the piece, ESC = do nothing, return"
  ito = -1
  CALL zeingabe(igt, 0, 1, ito, dummy, dummy$, mess$)
  IF ito = -1 THEN exit sub
...
SELECT CASE kdispl ' for actual pixels if larger screen resolution is used
  CASE 8,9,11,13,131,17,18,33, 331
...
CASE 13, 131, 33 'colored random dots, density proportional to level
  ...
  dxx = (js - ja + 1) * dx
  dyy = (ky - kay + 1) * dx
  IF kdispl <> 131 THEN 'with 131 no border
  ...
  FOR ix = ja TO js
    FOR iy = kay TO ky
      xb = xo + (ix - ja) * dx + 1
      yb = yo + (iy - kay) * dx + 1
      'light gray  background' plot only if inhibitor is > 0 = Working cell
      IF kdisplay = 131 AND axy(2, ix, iy) > 0 THEN LINE (xb, yb)-(xb + dx, yb + dx), 7, BF
      su = 0
      inuse = 0
      FOR il = 0 TO kn
      	IF ity(il) = 13 or ity(il) = 33 OR ity(il) = 131 THEN
	  inuse = inuse + 1
	  icolm(inuse) = ico(il)
