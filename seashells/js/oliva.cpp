...
float z[1200];
//start:
float it = 480;  //Number of lines to be calculated,
int i = 0;
float bf = 0, j = 0;
float tc = 0, af = 0, tbb = 0;
float aq3;
//          it*ip = Total number of iterations
float ip = 12;  //Number of iterations between the representations
float dx = 1;   //Cell width in pixels;   mit ip=12 ; kx=630 und dx=1 =>
int kx = 630;  //Number of cells               Simulation in a large field
//int dx = 2;
//int kx = 315, ip = 6;
float da = 0.015; //Diffusion of the activator
float ta = 0.1;  //Decay rate of the activator
float qa = 0.1;  //Basic production of the activator
float ra = 0.25; //Saturation of autocatalysis
float db = 0;    //Diffusion of the inhibitor
float tb = 0.014; //Decay rate of the inhibitor
float rb = 0.1;  //Michaelis-Menten constant of inhibition
float sb = 0.1;  //Life of the hormone
TRect Quelle, Ziel;
...
for (i=1; i<kx; i++) {
   ...
   z[i] = ta * (0.96 + 0.08 * random(1)); //Small fluctuation of autocatalysis
} //Next i
float aho = 0.5; //Hormone concentration
i = 10; for(j=1; j<20; j++) {  //initially activated cells, random
a[i] = 1; i = i + 20 * random(10) + 1; if (i > imax) j = 22;
} //Next
dac = 1 - ta - 2 * da; // These factors are always constant
float dbc = 1 - tb - 2 * db; // needed, they are
float dcc = 1 - tc - 2 * dc; // therefore calculated to start again
float dbcc = dbc;
dcc = dcc * 1;
/*
Screen 12
...
*/
int ic = 0;
float x1 = 50, y1 = 15; //Starting position of the graphic
for (float itot=0; itot>-1; itot++) {

    for (int iprint=1; iprint<ip; iprint++) { // Beginning of the iteration
        //Rem -----  --- render opaque
        float a1 = a[1];     //a1 is the concentration of a cell to the left of the current
        float b1 = a[1];     //Cell. Since the same concentration, no diffusion through the edge
        ...
        float bsa = 0;         //BSA => missing later sum of the activations of all cells
        // Rem ---------- reactions ------
        for (i=1; i<kx; i++) { // i = current cell kx = right cell
           float af = a[i]; //local activator concentration
           float bf = b[i]; //local inhibitory concentration
    ...
    //Rem ----------------Plot -------------
    kx = 630;
    if (ic == 0) Application->ProcessMessages(); //Do events
    ic++; if (ic==20) ic = 0;
    if (y1<500) {y1 = y1 + 1;} //Next deeper plot a pixel row
   ...
   for (int ix=1; ix<kx; ix++) {   //Pigment is drawn when over a threshold
