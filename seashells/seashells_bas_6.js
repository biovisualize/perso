...
function computeConcentrations ({
  ...
  const cellWidth = width / numCells // Pixel-size of a cell
    cellHeight = height / numExecutions 
  ...
  for(let i = 0; i<=numExecutions; i++) { // total number of plots
    ...
    for(let j = 0; j<=numIterations; j++) { // calculations between plots
      ...
      for (let k=0; k<=numCells; k++) { // k = current cell
        ...
        activatorX[k] = activatorConcentration + sourceDensity[k] * (activatorConcentration * activatorConcentration + activatorProductionRate) / (inhibitorConcentration + substanceCConcentration) - activatorRemovalRate * activatorConcentration + Diffactivator
        ihibitorX[k] = inhibitorConcentration + sourceDensity[k] * activatorConcentration * activatorConcentration - inhibitorRemovalRate * inhibitorConcentration + inhibitorDiffusion * ((inhibitorLeftCell - inhibitorConcentration) + (ihibitorX[k + 1] - inhibitorConcentration))
        if (substanceCRemovalRate > 0) { // c only in oscillating patterns and phyllotactivatorXis
          substanceCX[k] = substanceCConcentration + substanceCRemovalRate * activatorConcentration - substanceCRemovalRate * substanceCConcentration + substanceCDiffusion * ((substanceCLeftCell - substanceCConcentration) + (substanceCX[k + 1] - substanceCConcentration))
        }
...
computeConcentrations(presets[2])
