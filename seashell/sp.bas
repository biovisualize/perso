...
'''   -s gui 'compiler command to remove empty window
...
DECLARE SUB polygon (ihp, ix1, iy1, ipat, iwidth, ico, itypl)
DECLARE SUB prof (axt(), il, dw$, ipattern, inz, iendm, dx, ff, ja, js, iy, xo, yo)
...
DECLARE SUB showdisplay(ipcontrol, displaywas$)
...
'$INCLUDE: 'subs\fprofile.bas'
'$INCLUDE: 'subs\fdisplay.bas'

ifree = 1 ' a label for FreeBasic

imxl = 1451: ilm = 9: imx = 121: imy = 121
imyl = imxl
imxl2 = imxl / 2
DATA 10,22,55,75,115,180,195,240,310,352,375,425,455,555,585,595,630,700,750,780,932,999,1024,9999
programname$ = "sp"
basfilename$ = "sp.bas"
ptype$ = ""
dirtemp$ = "tmp\"
...
DIM axy(ilm, imx, imy), axt(ilm, imxl)'
...
IF ipcontrol = 0 THEN
    ...
    CASE 104, 105      ' for schematic display with F4, F5
...
CASE "S", "SM", "N", "C", "I", "II", "D", "CS"
  dra = 1! - ra - 2! * DA '- to save simulation time: these not necessary
  drb = 1! - rB - 2! * db '-   constant do not change during the simulations
  drc = 1! - rc - 2! * DC '-             ..
  drd = 1! - rd - 2! * DD '-             ..
  dre = 1! - re - 2! * DE '-             ..
  drf = 1! - rf - 2! * DF '-             ..
  ...
  FOR i = 1 TO imxl2
    IF pcontrol$ <> "II" THEN
      ...
      axt(0, i) = rho * (1! + KR / 100 * (RND - .5))'      * EXP(ad * i)'source density
      arandomxt(i) = 1! + KR / 100 * (RND - .5)
    END IF
    'RND: randomnumbers between 0.0 and 1.0 result in +-IR% fluctuation
    axt(1, i) = gA '-- initial condition: A is set to AA in all
    axt(2, i) = gB '-- cells, B, C, and D to AB, AC, and AD
  ...
  CASE 6 ' activation of cells, distance between 10 and 50 cells
    ix = RND * 30
    FOR i = 1 TO 20
 ...
 CASE 8 'Exponential gradient in the source density
    '    arteficial spatial periodic, stable in time,
    '    dy determines the difference between maxima and minima, wavelength = field.
    '    K2 is the phase;
    if k1 = 99 then ' circular field
      FOR i = 1 TO js:
      rho = ra: IF ca > 0 THEN rho = ca
      IF pcontrol$ <> "II" THEN  axt(0, i) = rho * (1! + 2 * KR / 100 * (RND - .5))
      axt(8, i) = 1! + dy * COS(((i - k2) / (js)) * 6.283) + ab * i / ky
    axt(0, i) = axt(0, i) * axt(8, i)
    axt(5, i) = re * (1 + ae * i / ky)
    NEXT i
  else
...
STEPPATTERN:
  ...
  CASE 11
    axt(8, ja) = aB  'left and right cell
    axt(8, js) = aB
 ...
 CASE 121 'for position-dependent gene-activation
 ...
 CASE 124: 'for gene activation in a single cell with threshold
     ...
     NEXT i 'Pos. Inf. in B
CASE 125 ' for border cells, gene activation by an inhibition of an inhibiton
 axt(3, ky / 2) = 1
 axt(3, ky / 2 + 1) = 1
...
CASE ELSE
  ...
  END SELECT
   itot = 0: igrowth = 1
refresh:
FOR il = 1 TO ilm'reset store array
    lax(il) = ja: lx(il) = js: lay(il) = 1
    FOR jx = 1 TO imx
      FOR jy = 1 TO imy
	axy(il, jx, jy) = 0
...
END IF 'K1 determines the position of insertion
END IF
...
CASE 211 '- Aktivator - Inhibitor Mechanismus: B is Inhibitor --------------
   'That is a special version of Eq. 21 to simulate regeneration in two parts
   ' left field ja to k4-1; right field k4 to js
...
CASE 91 '---  Clithon
   ...
   FOR i = ja TO js: GOSUB olddecay:
     ...
     axt(2, i) = olddecaydiffB - aq + bb
     cqf = C ^ 2
     REM arandomxt(i) = 1 plusm-minus random fluctuations (given by KR in %)
...
CASE 45 ' a,b= --Activator- Depletion system for pigment,
'       c, d = AI-system for stable patter
   FOR i = ja TO js: GOSUB olddecay:
...
CASE 85 '-- Branches versus triangles: antagonistic action of the
     ' c-d oscillation
     ' high d increases the decay rate, high c => longer half life of the inhibitor
     ' e is an additional inhibitor
   FOR i = ja TO js:
   drb = (1 - rbb - 2 * db)
   GOSUB olddecay:
...
CASE 121 ' Activation of several genes by a morphogen gradient
    'e = common repressor, f = positional information, "higher" genes = higher e
    'sa, sb, sc.. = efficiencies of the autoregulation
    ' axt(i,iy)= ra+(1 +- fluctuations
    ' cb changes the sclope of the gradient
    ' f = Positional information
    ' ba determines the absolute level of the Pos. Information
   FOR i = ja TO js: GOSUB olddecay' :
  ...
  CASE 123 'Activation of different genes within one cell
'  i=>Gen Nr., NOT POSITION
   FOR i = ja TO js: GOSUB olddecay:
	aq = a * a / (1! + sA * a * a)
	axt(2, i) = axt(8, i) * ba
	...
	END IF
   NEXT i
CASE 124 'only one gene becomes activated if positional information b is above threshold
   FOR i = ja TO js: GOSUB olddecay:
...
CASE 135 'GC-- two antagonists, for chemotactic orientation of cells
'  b=uniformely distributed inhibitor s'=s/(sc+c)
   FOR i = ja TO js: GOSUB olddecay
  axt(1, i) = olddecaydiffA + s / (sc + cc * c) * (A * A / B + ba) / (1 + sa * A * A)
  axt(3, i) = olddecaydiffC + bc * A
...
' ftz is an further system
     eq = re * arandomxt(js+1-i) * f * (e * e + be + sf * C) / (1 + ce * a)
 ...
 END SELECT
  ...
  IF imanipulate = K3 THEN
    ...
    CASE 16, 17
      mess$ = "External gradient (s; blue) will be changed (k2 position, DZ change pos."
    ...
    CALL gtreturn(0, 0)
    END SELECT
    CALL cmessage(6, -1, 1, 15, 2, ibackcol, mess$, "OK")
  END IF
    GOTO simulation
...
CALL manipulationxt(pcontrol2$, kinsert)
  CALL showprof(KD, 1, displaywas$, axt(), ja, js, icc)
''  IF ipcx > -1 THEN CALL PCXOUT(ipcx, ipcs) not used in FreeBasic
iparam = 2: ipcontrol = 0
END SELECT
GOTO parameter
