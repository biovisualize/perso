...
var ta = 0.05;  //Decay rate of the activator
var qa = 0.01;  //Basic production of the activator
var ra = 0.02; //Saturation of autocatalysis
var db = 0.001;    //Diffusion of the inhibitor
var tb = 0.003; //Decay rate of the inhibitor
var rb = 0.05;  //Michaelis-Menten constant of inhibition
var sb = 0.03;  //Life of the hormone

var range = Array.from(Array(400).keys())
...
var a1, b1, af, bf;
//for (y1=15; y1<500; y1++) {
function renderRow(y1) {
    for (var iprint=1; iprint<ip; iprint++) { // Beginning of the iteration
        //Rem -----  --- render opaque
        a1 = a[1];     //a1 is the concentration of a cell to the left of the current
        b1 = a[1];     //Cell. Since the same concentration, no diffusion through the edge
...
//        tbb = 1;//'.5;
    ...
    var queue = renderSlicer(renderRow).rate(10);
    queue(range);
